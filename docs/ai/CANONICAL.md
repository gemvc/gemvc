...
Framework hub: **gemvc/library 5.16.0**.
...
**Family trust (machine):** `$this->requireInternalService()` — HMAC; env `GEMVC_INTERNAL_SECRET`.  
**Mesh caller:** `ServiceCall::to('auth')->post(...)->withInternalTrust()->run()` — `GEMVC_SERVICES_JSON`. See [http-client.md](../guides/http-client.md#servicecall-phase-2b).
