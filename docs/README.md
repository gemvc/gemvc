...
Narrative notes (what/why/migration), including **5.16.0** ServiceCall mesh DX, **5.15.0** family trust, **5.14.0** FrankenPHP / `StandardHttpRequest`, **5.13.0** unified `ApiService`, **5.12.0** rate-limit drivers / Protected API / `forUpdate`, **5.11.0 ViewTable** / `db:migrate --all`, 5.9.x multi-DB, decimal, cli-dev, `requireAuth`.
