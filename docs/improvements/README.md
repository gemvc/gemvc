...
| **Unified `ApiService` (runtime)** | Phase 1 **done** (validation); Phase 2–3 planned | `validateOrFail()` / `validateStringOrFail()` on both bases; SwooleBootstrap catches `ValidationException`. Keep dual bases until shared behavior extract. See [api-runtime-unification.md](api-runtime-unification.md) |
