...
class UniversalQueryExecuter
{
    ...
    public function execute(): bool
    {
        ...
        // APM integration: Start database query span if enabled
        ...
        if (ProjectHelper::isApmEnabled() !== null) {
            // Get the APM instance (can work without Request for database tracing)
            $apm = ApmFactory::create(null);
            
            if ($apm !== null && $apm->isEnabled() && $apm->shouldTraceDbQuery()) {
                $shouldTrace = true;
                
                // Determine query type from first word (optimized with match expression)
                $queryUpper = strtoupper(ltrim($this->query));
        ...
        try {
            ...
            // APM: End span with success details
            ...
            if ($shouldTrace && !empty($dbSpan) && $apm !== null) {
                ...
                $apm->endSpan($dbSpan, [
                    'db.rows_affected' => '0',
                    'db.execution_time_ms' => (string)$executionTime,
                    'db.error_code' => (string)$e->getCode(),
